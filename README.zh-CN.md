...
Desk OCR 可以在鼠标所在屏幕拖动选择截图区域，或打开本地图片，在本机运行 PaddleOCR，并在原图上绘制文本框、叠加可选择文字层，支持搜索、高亮与复制。图片和识别结果不会上传到云端。
...
- 使用 `Command/Ctrl + Shift + O` 在鼠标所在屏幕拖动选择截图区域；按 `Esc` 或单击右键可取消。
...
Electron 窗口会自动打开。点击“截图并 OCR”或按 `Command/Ctrl + Shift + O`，然后拖动选择要识别的区域；也可以点击“导入图片”使用已有图片。首次启动默认使用中文，可通过“设置 → 界面语言”切换为英文。语言选择只保存在本机，不会改变截图或 OCR 结果。
